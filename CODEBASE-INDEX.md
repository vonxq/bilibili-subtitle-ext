...
├── src/
│   ├── inject.js                           (93行)   # 页面上下文脚本，拦截 fetch/XHR
│   ├── background/
...
- **职责**: 语言设置覆盖层（母语/目标语选择）
- **API**:
  - `create(onChange)` → HTMLElement，onChange(nativeLang, targetLang)
...
- **拦截规则**:
  - URL 包含 `ai_subtitle` → 派发 `bili-subtitle-data` 事件
